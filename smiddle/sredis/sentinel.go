...
type Sentinel struct {
	// Address is a slice with known Sentinel addresses.
	Address []string
...
func (s *Sentinel) putToTop(addr string) {
	...
	for _, a := range address {
		...
		newAddress = append(newAddress, a)
	}
...
// MasterAddress returns an address of current Redis master instance.
func (s *Sentinel) MasterAddress() (string, error) {
...
// SlaveAddress returns a slice with known slave addresses of current master instance.
func (s *Sentinel) SlaveAddress() ([]string, error) {
	res, err := s.doUntilSuccess(func(c redis.Conn) (interface{}, error) {
...
func (s *Sentinel) Discover() error {
	...
	for _, addr := range addrs {
		if !stringInSlice(addr, s.Address) {
			s.Address = append(s.Address, addr)
		}
	}
	s.mu.Unlock()
	return nil
...
func getRole(c redis.Conn) (string, error) {
	...
	temp, ok := res.([]interface{})
	if ok {
		return redis.String(temp[0], nil)
	}
...
func queryForSlaveAddrs(conn redis.Conn, masterName string) ([]string, error) {
	...
	slaveAddress := make([]string, 0)
	for _, slave := range slaves {
		slaveAddress = append(slaveAddress, slave.Addr())
	}
	return slaveAddress, nil
}
